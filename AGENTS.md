...
- Trajectory encoder artifacts for tracking should be checkpoint-first on W&B: upload `best_model.pt`, `last_model.pt`, `config.yaml`, and `normalizer.pt`, and treat local `best_encoder.jit` exports as optional deploy-time outputs rather than the canonical shared artifact.
- `env.commands.motion.encoder_dir` may point at either a local/exported `.jit` file or a checkpoint artifact such as `wandb://entity/project/artifact:alias/best_model.pt`. The multitracking command now infers encoder horizon from the sidecar metadata and validates latent dimension against the restored encoder output.
- Keep W&B artifact downloads on the same writable cache roots used by the Slurm overlay environment: prefer `MJLAB_WANDB_CACHE_DIR`, then `WANDB_ARTIFACT_DIR`, rather than relying on W&B defaults inside the container.
