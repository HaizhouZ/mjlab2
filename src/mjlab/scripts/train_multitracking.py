...
Usage:
    uv run src/mjlab/scripts/train_multitracking.py Mjlab-MultiTracking-Flat-Unitree-G1
    ...
    # Using multiple GPUs
    CUDA_VISIBLE_DEVICES=0,1 uv run src/mjlab/scripts/train_multitracking.py Mjlab-MultiTracking-Flat-Unitree-G1\
        --registry-names wandb-registry-motions/motion1 wandb-registry-motions/motion2 \
        --gpu-ids all
...
def run_train_single_motion_from_registry(
  ...
  # maybe set the artifact directory here?
  ...
  if isinstance(motion_cmd, MotionCommandCfg):
    ...
    # this will set motion_dir to artifacts (should be controlled) and motion_name_pattern to single motion
...
def launch_training_multi_motion(cfg: TrainMultiMotionConfig) -> None:
  ...
  for i, (motion_name, registry_name, motion_file) in enumerate(motions_to_train):
    ...
    # Create nested log directory structure: {log_root}/{experiment_name}/{motion_name}/{timestamp}
    # This prevents overwriting while keeping motion name for wandb
    log_dir = get_log_dir(
      experiment_name=f"{experiment_name}/{motion_name}", run_name=cfg.run_name_suffix
    )
    ...
    if num_gpus <= 1:
      ...
      if registry_name:
        ...
        raise NotImplementedError("Local motion file training not implemented.")
