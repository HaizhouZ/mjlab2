...
def _extract_last_frame_yaw(
  ...
  with np.load(input_file, allow_pickle=True) as data:
    assert "qpos" in data, (
      f"Key 'qpos' not found in OmniRetarget NPZ file: {input_file}"
    )
    qpos_np = data["qpos"].astype(np.float32)  # (T, 43)
    # Extract base quaternion: qpos[:, 0:4] = [qw, qx, qy, qz]
    base_quats = qpos_np[:, 0:4]
...
def _extract_first_frame_base_pos(
  ...
  with np.load(input_file, allow_pickle=True) as data:
    ...
    qpos_np = data["qpos"].astype(np.float32)  # (T, 43)
    # Extract base position: qpos[:, 4:7] = [x, y, z]
    base_pos = qpos_np[:, 4:7]
...
def _get_input_fps(input_file: str) -> int:
  """Get input FPS from OmniRetarget file without full loading."""
  with np.load(input_file, allow_pickle=True) as data:
    if "fps" in data:
      return int(data["fps"])
