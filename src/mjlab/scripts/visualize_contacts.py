...
def set_robot_state(
  ...
  root_state = torch.from_numpy(
    np.concatenate([root_pos, root_quat, root_lin_vel, root_ang_vel])
  ).float().unsqueeze(0).to(device)  # (1, 13)
  
  robot.write_root_state_to_sim(root_state)
  
  # Set joint positions and velocities
  joint_pos = torch.from_numpy(frame_data["joint_pos"]).float().unsqueeze(0).to(device)  # (1, n_joints)
  
  if "joint_vel" in frame_data:
    joint_vel = torch.from_numpy(frame_data["joint_vel"]).float().unsqueeze(0).to(device)  # (1, n_joints)
  else:
...
def set_object_state(
  ...
  root_pose = torch.from_numpy(
    np.concatenate([obj_pos, object_quat])
  ).float().unsqueeze(0).to(device)  # (1, 7)
...
def visualize_contacts(
  ...
  has_contacts = "contact_positions" in motion_data and "contact_indicators" in motion_data
  if has_contacts:
    ...
    print(f"Contact data found: {total_contacts} contact detections across {num_eefs} end effectors")
  else:
  ...
  if show_contact_markers and has_contacts:
    ...
    for eef_idx in range(num_eefs):
      ...
      site = scene.spec.worldbody.add_site(
        ...
        rgba=(1.0, 0.0, 0.0, 0.8) if eef_idx == 0 else (0.0, 0.0, 1.0, 0.8),  # Red for left, blue for right
  ...
  with mujoco.viewer.launch_passive(mj_model, mj_data, key_callback=key_callback) as viewer:
    ...
    while viewer.is_running():
      ...
      env_origin = scene.env_origins[0].cpu().numpy() if hasattr(scene, "env_origins") else None
      ...
      if has_object and box is not None:
        ...
        set_object_state(box, object_pos, object_quat, device=device, env_origin=env_origin)
        ...
        if frame_idx % 50 == 0:
          ...
          for eef_idx in range(num_eefs):
            if contact_indicators[frame_idx, eef_idx]:
              contact_pos = contact_positions[frame_idx, eef_idx]
              if not np.isnan(contact_pos).any():
                eef_name = eef_names[eef_idx] if eef_idx < len(eef_names) else f"EEF {eef_idx}"
                contact_info.append(f"{eef_name}: [{contact_pos[0]:.3f}, {contact_pos[1]:.3f}, {contact_pos[2]:.3f}]")
          
          if contact_info:
            print(f"Frame {frame_idx}/{num_frames}: Contacts - {', '.join(contact_info)}")
